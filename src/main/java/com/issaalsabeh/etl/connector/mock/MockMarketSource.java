...
public class MockMarketSource implements Source<MarketEvent> {
    ...
    private static final String[] SYMBOLS = {"AAPL", "MSFT", "NVDA","TSLA"};
    ...
    public MarketEvent poll() {
        ...
        long volume = ThreadLocalRandom.current().nextLong(100, 100_000);

        UUID eventId = UUID.randomUUID();

        Instant timestamp = Instant.now();

        return new MarketEvent(eventId, symbol, price, volume, timestamp);
    }
